...
class Server:
    ...
    def initialize_matrix(self):
        for i in range (19) :
            self.game2_white_matrix.append ([])
            self.game2_black_matrix.append ([])
            for j in range (19) :
                self.game2_white_matrix[i].append ([0, 0])
                self.game2_black_matrix[i].append ([0, 0])


    def record_game2_board(self , pos , color):
        print(self.game2_black_matrix)
        ...
        if within_the_board :
            ...
            if color == [0 ,0 ,0] :
                self.game2_black_matrix[times2][times1][0] = grid_x
                self.game2_black_matrix[times2][times1][1] = grid_y
                self.game2_color_matrix[times2][times1] = -1
            else :
                self.game2_white_matrix[times2][times1][0] = grid_x
                self.game2_white_matrix[times2][times1][1] = grid_y
                self.game2_color_matrix[times2][times1] = 1
...
# ==============================================================================
            ...
            elif msg['action'] == 'game2':
                peer = msg['with']
                to_sock = self.logged_name2sock[peer]
                mysend(to_sock , json.dumps({'action' : 'game2' , 'from' : msg['from']  }  ))
                ...
                color = msg['color']
                if msg['position'] != '':
                    self.record_game2_board(pos , color)
                self.game2_state = msg['state']
                if msg['color'] == [0, 0, 0]:
                    self.player1 = msg['one more']
                if msg['color'] == [255, 255, 255]:
                    self.player2 = msg['one more']
                if self.game2_state == 'over' and self.player1 != '' and self.player2 != '':
                    self.game2_white_matrix = []
                    self.game2_black_matrix = []
                    self.initialize_matrix()
                    self.game2_color_matrix = [[0] * 19 for i in range (19)]
                    self.game2_state = ''
            elif msg['action'] == 'refresh':
                mysend(from_sock , json.dumps({'state' : self.game2_state ,'white' : self.game2_white_matrix , 'black' : self.game2_black_matrix , 'color' : self.game2_color_matrix , 'player1':self.player1 , 'player2': self.player2}))
